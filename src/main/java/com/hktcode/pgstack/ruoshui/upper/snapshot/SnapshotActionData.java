...
abstract class SnapshotActionData //
    extends TqueueAction<SnapshotAction, SnapshotConfig, UpcsmFetchRecordSnapshot> //
    implements SnapshotAction //
{
    private static final Logger logger = LoggerFactory.getLogger(SnapshotActionData.class);
    ...
    protected <T extends SnapshotActionData> //
    SnapshotActionData(T action, long actionStart)
    {
    ...
    abstract SnapshotAction next(ExecutorService exesvc, PgConnection pgdata, PgConnection pgrepl) //
    ...
    public abstract SnapshotMetricRun toRunMetrics();

    @Override
    public SnapshotResult get()
    {
        return SnapshotResultRun.of(this.config, this.toRunMetrics());
    }

    @Override
    public SnapshotResultEnd del()
    {
        return SnapshotResultEnd.of(this.config, this.toEndMetrics());
    }
}
