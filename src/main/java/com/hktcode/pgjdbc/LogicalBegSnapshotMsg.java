 ...
 * 这个消息是本程序在获取快照时发出的。
 */
public class LogicalBegSnapshotMsg implements LogicalMsg
{
    /**
     * 根据指定的关系列表构建{@link LogicalBegSnapshotMsg}对象.
     *
     ...
     */
    public static LogicalBegSnapshotMsg of(ImmutableList<PgReplRelation> relalist)
    {
        if (relalist == null) {
            throw new ArgumentNullException("relalist");
        }
        return new LogicalBegSnapshotMsg(relalist);
    }
     ...
     */
    private LogicalBegSnapshotMsg(ImmutableList<PgReplRelation> relalist)
    {
